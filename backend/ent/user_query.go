...
// QueryLotteryDrawRecords chains the current query on the "lottery_draw_records" edge.
func (_q *UserQuery) QueryLotteryDrawRecords() *LotteryDrawRecordQuery {
...
// WithLotteryDrawRecords tells the query-builder to eager-load the nodes that are connected to
// the "lottery_draw_records" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithLotteryDrawRecords(opts ...func(*LotteryDrawRecordQuery)) *UserQuery {
	query := (&LotteryDrawRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withLotteryDrawRecords = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withLotteryDrawRecords; query != nil {
		if err := _q.loadLotteryDrawRecords(ctx, query, nodes,
...
func (_q *UserQuery) loadLotteryDrawRecords(ctx context.Context, query *LotteryDrawRecordQuery, nodes []*User, init func(*User), assign func(*User, *LotteryDrawRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(lotterydrawrecord.FieldUserID)
	}
