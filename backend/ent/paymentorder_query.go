...
func (_q *PaymentOrderQuery) QueryCarpoolParticipants() *CarpoolParticipantQuery {
...
func (_q *PaymentOrderQuery) WithCarpoolParticipants(opts ...func(*CarpoolParticipantQuery)) *PaymentOrderQuery {
	query := (&CarpoolParticipantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withCarpoolParticipants; query != nil {
		if err := _q.loadCarpoolParticipants(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.CarpoolParticipants = []*CarpoolParticipant{} },
			func(n *PaymentOrder, e *CarpoolParticipant) {
...
func (_q *PaymentOrderQuery) loadCarpoolParticipants(ctx context.Context, query *CarpoolParticipantQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *CarpoolParticipant)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(carpoolparticipant.FieldPaymentOrderID)
	}
	query.Where(predicate.CarpoolParticipant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.CarpoolParticipantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
