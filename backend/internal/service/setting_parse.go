...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		...
		AliyunSMSAccessKeySecret:           strings.TrimSpace(settings[SettingKeyAliyunSMSAccessKeySecret]),
		...
		AliyunSMSRegistrationMode:          normalizeAliyunSMSRegistrationMode(settings[SettingKeyAliyunSMSRegistrationMode]),
		AliyunSMSVerifyCodeSignName:        strings.TrimSpace(settings[SettingKeyAliyunSMSVerifyCodeSignName]),
		AliyunSMSVerifyCodeTemplateCode:    strings.TrimSpace(settings[SettingKeyAliyunSMSVerifyCodeTemplateCode]),
		AliyunSMSVerifyCodeStaticParams:    strings.TrimSpace(settings[SettingKeyAliyunSMSVerifyCodeStaticJSON]),
		AliyunSMSTemplateCode:              strings.TrimSpace(settings[SettingKeyAliyunSMSTemplateCode]),
		AliyunSMSTemplateParamKey:          strings.TrimSpace(settings[SettingKeyAliyunSMSTemplateParamKey]),
		AliyunSMSTemplateStaticParams:      strings.TrimSpace(settings[SettingKeyAliyunSMSTemplateStaticJSON]),
	...
	if seconds, err := strconv.Atoi(settings[SettingKeyAliyunSMSValidTimeSeconds]); err == nil && seconds > 0 {
		result.AliyunSMSValidTimeSeconds = seconds
	} else {
		result.AliyunSMSValidTimeSeconds = 300
	}
	if seconds, err := strconv.Atoi(settings[SettingKeyAliyunSMSIntervalSeconds]); err == nil && seconds > 0 {
		result.AliyunSMSIntervalSeconds = seconds
	} else {
		result.AliyunSMSIntervalSeconds = 60
	}
