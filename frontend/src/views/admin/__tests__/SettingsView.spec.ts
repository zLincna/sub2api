...
async function openSmsTab(wrapper: ReturnType<typeof mountView>) {
  const smsTabButton = wrapper
    .findAll("button")
    .find((node) => node.text().includes("admin.settings.tabs.sms"));

  expect(smsTabButton).toBeDefined();
  await smsTabButton?.trigger("click");
  await flushPromises();
}
...
describe("admin SettingsView registration verification controls", () => {
  ...
  it("opens Aliyun SMS settings from the phone verification option", async () => {
    ...
    await flushPromises();
    await openSmsTab(wrapper);

    expect(wrapper.text()).toContain("短信配置");
    const accessKeyInput = wrapper.get<HTMLInputElement>(
      '[data-testid="aliyun-sms-access-key-id"]',
    );
    const secretInput = wrapper.get<HTMLInputElement>(
      '[data-testid="aliyun-sms-access-key-secret"]',
    );
    expect(accessKeyInput.element.value).toBe("old-ak");
    expect(secretInput.element.placeholder).toContain("密钥已配置");

    await accessKeyInput.setValue("new-ak");
